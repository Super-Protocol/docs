...
RELEASE_NAME="${RELEASE_NAME:-medgemma-official}"
API_KEY="${API_KEY:-vllm-local}"
...
if [ -z "${HF_TOKEN:-}" ]; then
  echo "HF_TOKEN is required for ${MODEL_NAME}." >&2
...
echo
echo "==> Waiting for MedGemma pod readiness..."
