...
contract DeployLiquity2Script is DeployGovernance, UniPriceConverter, StdCheats, MetadataDeployment, Logging {
    ...
    uint256 constant NUM_BRANCHES = 1;

    address WETH_ADDRESS = 0x4200000000000000000000000000000000000006;
    address USDC_ADDRESS = 0x9e5AAC1Ba1a2e6aEd6b32689DFcF62A509Ca96f3;
    ...
    address ETH_ORACLE_ADDRESS = 0xdBB75D7E0F363450a94B472B1EfCe7AA1837bDc2;
    ...
    // Mainnet
    ISwapRouter constant uniV3RouterMainnet = ISwapRouter(0x1b81D678ffb9C0263b24A97847620C99d213eB14);
    IQuoterV2 constant uniV3QuoterMainnet = IQuoterV2(0xB048Bbc1Ee6b733FFfCFb9e9CeF7375518e25997);
    IUniswapV3Factory constant uniswapV3FactoryMainnet = IUniswapV3Factory(0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865);
    ...
    function run() external {
        ...
        uint256 epochStart = vm.envOr(
            "EPOCH_START",
            (block.chainid == 204 ? _latestUTCMidnightBetweenWednesdayAndThursday() : block.timestamp) - EPOCH_DURATION
        );
        ...
        if (block.chainid == 204) {
            ...
            // lusd = LUSD_ADDRESS;
            lqty = address(new ERC20Faucet("Liquity", "LQTY", 0, type(uint256).max));
            lusd = address(new ERC20Faucet("Liquity USD", "LUSD", 0, type(uint256).max));
    ...
    function _deployAndConnectContracts(
        ...
        // Collaterals
        if (block.chainid == 204 && !useTestnetPriceFeeds) {
            // mainnet
            // ETH
