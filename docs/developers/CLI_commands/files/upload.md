...
```
./spctl files upload targzArchivePath [option ...]
```

Replace `targzArchivePath` with the path to the uploading TAR.GZ archive file.
...
| `--storage` | Option to use a storage offer. State the storage offer ID and the requirement slot ID, separated by a comma. |
...
./spctl files upload ./data/content.tar.gz \
    --filename ./fileData.tar.gz \
    --output ./fileResource.json \
