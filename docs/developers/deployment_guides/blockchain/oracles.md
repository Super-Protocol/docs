...
Теперь мы можем наблюдать за работой оракла в реальном времени. Откройте страницу контракта dApp на Polygonscan `https://mumbai.polygonscan.com/address/<d-app-address>`, по адресу контракта, который мы получили на шаге `Deploy dApp`. На вкладке `Contract` -> `Read Contract` нажмите на методы:
